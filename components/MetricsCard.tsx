    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-2xl font-bold leading-tight ${alert ? 'text-red-900 dark:text-red-200' : 'text-gray-900 dark:text-white'}`}>
            {value}
          </p>
        </div>
        ...
        {icon && (
          ...
          >
            {alert && <AlertCircle className="w-5 h-5 text-red-600 dark:text-red-300" />}
            {!alert && (
              <div className="w-5 h-5 opacity-70 text-gray-600 dark:text-gray-400">
                {icon}
              </div>
            )}
          </div>
